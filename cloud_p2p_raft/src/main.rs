...
impl FailureSimulator {
    ...
    pub async fn simulate_failure(&self) {
        ...
        *self.failure_start.write().await = Some(Instant::now());
        info!(
            "🔥 Node entering failure state for {} seconds",
            self.failure_duration.as_secs()
        );
    }
...
impl RaftNode {
    ...
    async fn election_timeout_loop(&self) {
        loop {
            ...
            if matches!(state, RaftState::Follower) {
                ...
                info!(
                    "Node {}: Candidate timed out, stepping down to follower",
                    self.id
                );
    ...
    async fn handle_append_entries(
        ...
        if term >= *current_term {
            ...
            *self.last_heartbeat.write().await = Instant::now();
            ...
            if prev_log_index == 0
                || (log.len() as u64 >= prev_log_index
                    && (prev_log_index == 0
                        || log.get((prev_log_index - 1) as usize).map(|e| e.term)
                            == Some(prev_log_term)))
            {
                // Append new entries
                ...
                if !entries.is_empty() {
                    info!(
                        "Node {} received {} log entries from leader {}",
                        self.id,
                        entries.len(),
                        leader_id
                    );
    ...
    async fn sync_state_from_peers(&self) {
        ...
        *self.voted_for.write().await = None;
        self.votes_received.write().await.clear();
        info!(
            "Node {} forcibly stepped down to follower for term {} after recovery",
            self.id, *current_term
        );
        info!(
            "Node {} synchronized term to {} after recovery",
            self.id, *current_term
        );
    }
...
async fn main() {
    ...
    for node in [&node1, &node2, &node3] {
        ...
        info!(
            "Node {}: State={:?}, Term={}, VotedFor={:?}",
            node.id, state, term, voted_for
        );
    }
    ...
    for node in [&node1, &node2, &node3] {
        ...
        if matches!(state, RaftState::Leader) {
            ...
            let _ = node
                .register_user("alice".to_string(), "192.168.1.100".to_string())
                .await;
            let _ = node
                .register_user("bob".to_string(), "192.168.1.101".to_string())
                .await;
    ...
    for i in 0..12 {
        ...
        for node in [&node1, &node2, &node3] {
            ...
            info!(
                "Node {}: State={:?}, Term={}, Status={}",
                node.id, state, term, status
            );
        }
    }
