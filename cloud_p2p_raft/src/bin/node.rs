...
// use cloud_p2p_raft::utils::stego::{stego_wrap, stego_unwrap};
// use tokio::fs;
// use std::path::Path;
// use anyhow::Context;
...
//     // create photos dir
//     tokio::fs::create_dir_all("photos").await?;

//     let tmp_path = format!("photos/{}.part", photo_id);
//     let raw_path = format!("photos/{}.bin", photo_id);
//     let stego_path = format!("photos/{}.png", photo_id);
...
//         if n == 0 {
//             anyhow::bail!("unexpected EOF while reading photo (remaining {})", remaining);
//         }
...
//     tokio::fs::rename(&tmp_path, &raw_path).await?;

//     // Perform stego embedding
//     let cover_path = "covers/default.png";
//     if !Path::new(cover_path).exists() {
//         let _ = tokio::fs::remove_file(&raw_path).await;
//         anyhow::bail!(
//             "cover image not found at '{}'. Please place a PNG cover at that path",
//             cover_path
//         );
//     }

//     // read raw secret to memory synchronously (we spawn blocking)
//     let raw_path_clone = raw_path.clone();
// let stego_path_clone = stego_path.clone();
// let cover_path_clone = cover_path.to_string();

// let embed_checksum = tokio::task::spawn_blocking(move || -> anyhow::Result<String> {
//     // read cover bytes
//     let cover_bytes = std::fs::read(&cover_path_clone)
//         .with_context(|| format!("Failed to read cover image '{}'", cover_path_clone))?;
//     // read secret bytes
//     let secret_bytes = std::fs::read(&raw_path_clone)
//         .with_context(|| format!("Failed to read raw secret '{}'", raw_path_clone))?;
//     // apply stego embedding
//     let out_bytes = stego_wrap(&cover_bytes, &secret_bytes)
//         .with_context(|| "Stego wrap failed")?;
//     // write the stego PNG
//     std::fs::write(&stego_path_clone, &out_bytes)
//         .with_context(|| format!("Failed to write stego image '{}'", stego_path_clone))?;
//     // Optionally verify by unwrapping
//     let extracted_bytes = stego_unwrap(&out_bytes)
//         .with_context(|| "Stego unwrap failed")?;
//     // compute checksum of extracted_bytes or convert as hex string if needed
//     let extracted_checksum = hex::encode(sha2::Sha256::digest(&extracted_bytes));
//     Ok(extracted_checksum)
// })
// .await
// .context("spawn_blocking join error")??;

//     let embed_checksum_hex = hex::encode(&embed_checksum);

    
//     if embed_checksum != checksum_hex {
//     tracing::warn!(
//         "checksum mismatch after stego embed/unembed: raw={} embed_checksum={}",
//         checksum_hex,
//         embed_checksum,
//     );
// }

//     // optionally clean up raw file
//     let _ = tokio::fs::remove_file(&raw_path).await;
...
enum ClientRequest {
    ...
    GetLeader,
    GetState,
    UploadImage { photo_id: String, file_path: String },
}
...
enum ClientResponse {
    ...
    State { role: String, term: u64, commit_index: u64 },
    Error { message: String },
    UploadSuccess { photo_id: String, checksum: String },
}
...
struct NetNode {
    ...
    registered_users: Arc<RwLock<HashMap<String, String>>>,
    processed_ops: Arc<RwLock<std::collections::HashSet<String>>>,
    image_storage: Arc<RwLock<HashMap<String, Vec<u8>>>>,
}
...
impl NetNode {
    pub fn new(id: u32, peers: HashMap<u32, SocketAddr>) -> Self {
        ...
        Self {
            ...
            registered_users: Arc::new(RwLock::new(HashMap::new())),
            processed_ops: Arc::new(RwLock::new(std::collections::HashSet::new())),
            image_storage: Arc::new(RwLock::new(HashMap::new())),
        }
    }
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    if matches!(*input_node.state.read().await, RaftState::Leader) {
                        ...
                        log.push(LogEntry { 
                            term, 
                            index, 
                            command: LogCommand::UserOp(line.clone()) 
                        });
                        drop(log);

                        info!("Node {}: appended new entry {}", input_node.id, line);
                        input_node.broadcast_append_entries().await;
                    } else {
    ...
    async fn handle_message(&self, message: RaftMessage) {
        match message {
            ...
            RaftMessage::ImageData { photo_id, data, sender_id } => {
                info!("Node {} received image data for {} ({} bytes) from {}", 
                     self.id, photo_id, data.len(), sender_id);
                self.store_image_locally(&photo_id, &data).await;
            }
        }
    }
                            ...
                            match parts.next() {
                                Some("REGISTER") => {
                                    if let (Some(user), Some(ip)) = (parts.next(), parts.next()) {
                                        ...
                                        info!("Node {}: Malformed REGISTER command '{}'", self.id, cmd);
                                    }
                                }
                                Some("UNREGISTER") => {
                                    if let Some(user) = parts.next() {
                                        ...
                                        info!("Node {}: Malformed UNREGISTER command '{}'", self.id, cmd);
                                    }
                                }
                                Some(other) => {
                                    info!("Node {}: Applied user command '{}'", self.id, other);
                                }
                                None => {}
                            }
                        }
                        LogCommand::StoreImage { photo_id, size, checksum } => {
                            info!("Node {}: Applied STORE_IMAGE {} ({} bytes, checksum {})", 
                                 self.id, photo_id, size, checksum);
                            // Image data should already be stored via ImageData messages
                        }
                    }
                }
            }
    ...
    async fn store_image_locally(&self, photo_id: &str, data: &[u8]) {
        let mut storage = self.image_storage.write().await;
        storage.insert(photo_id.to_string(), data.to_vec());
        info!("Node {}: Stored image {} ({} bytes)", self.id, photo_id, data.len());
    }

    async fn distribute_image_to_peers(&self, photo_id: &str, data: &[u8]) {
        for (&peer_id, _) in self.peers.iter() {
            let msg = RaftMessage::ImageData {
        ...
        info!("Node {}: Distributed image {} to all peers", self.id, photo_id);
    }

    async fn process_image_upload(&self, photo_id: &str, file_path: &str) -> anyhow::Result<String> {
        // Read image file
        let image_data = tokio::fs::read(file_path).await
            .with_context(|| format!("Failed to read image file: {}", file_path))?;
        
        // Calculate checksum
        let checksum = hex::encode(Sha256::digest(&image_data));
        
        // Store locally
        self.store_image_locally(photo_id, &image_data).await;
        
        // Distribute to peers
        self.distribute_image_to_peers(photo_id, &image_data).await;
        
        // Create stego version if cover image exists
        self.create_stego_version(photo_id, &image_data).await?;
        
        Ok(checksum)
    }

    async fn create_stego_version(&self, photo_id: &str, data: &[u8]) -> anyhow::Result<()> {
    let cover_path = "covers/default.png";
    if !Path::new(cover_path).exists() {
        warn!("Cover image not found at '{}', skipping stego creation", cover_path);
        return Ok(());
    }

    // Clone photo_id before moving into closure
    let photo_id_clone = photo_id.to_string();
    let data = data.to_vec();
    let cover_path = cover_path.to_string();

    tokio::task::spawn_blocking(move || -> anyhow::Result<()> {
        let cover_bytes = std::fs::read(&cover_path)
            .with_context(|| format!("Failed to read cover image '{}'", cover_path))?;
        
        let out_bytes = stego_wrap(&cover_bytes, &data)
            .with_context(|| "Stego wrap failed")?;
        
        let stego_path = format!("photos/{}.png", photo_id_clone);
        std::fs::write(&stego_path, &out_bytes)
            .with_context(|| format!("Failed to write stego image '{}'", stego_path))?;
        
        Ok(())
    }).await??;

    info!("Created stego version for image {}", photo_id);
    Ok(())
    }
    ...
    async fn handle_client_connection(&self, mut stream: TcpStream) -> anyhow::Result<()> {
        ...
        w.write_all(b"  LEADER\n").await?;
        w.write_all(b"  REGISTER <user> <ip>\n").await?;
        w.write_all(b"  UNREGISTER <user>\n").await?;
        w.write_all(b"  LIST\n").await?;
        w.write_all(b"  SHOW_USERS\n").await?;
        w.write_all(b"  SUBMIT <op_id> <command...>\n").await?;
        w.write_all(b"  UPLOAD_IMAGE <photo_id> <file_path>\n").await?;
        w.write_all(b"  SHOW_IMAGES\n").await?;
        ...
        loop {
            ...
            match parts.next() {
                ...
                Some("REGISTER") => {
                    ...
                    let (user, ip) = match (parts.next(), parts.next()) {
                        (Some(user), Some(ip)) => (user.to_string(), ip.to_string()),
                        _ => {
                            w.write_all(b"Usage: REGISTER <user> <ip>\n").await?;
                            continue;
                        }
                    ...
                    let command = format!("REGISTER {} {}", user, ip);
                    self.append_and_replicate(LogCommand::UserOp(command)).await;
                    w.write_all(b"OK\n").await?;
                }
                ...
                Some("LIST") => {
                    ...
                    if log.is_empty() {
                        ...
                        for e in log.iter() {
                            let cmd_str = match &e.command {
                                LogCommand::UserOp(s) => s.clone(),
                                LogCommand::StoreImage { photo_id, size, checksum } => 
                                    format!("STORE_IMAGE {} ({} bytes, checksum {})", photo_id, size, checksum),
                            };
                            let s = format!("idx={} term={} cmd={}\n", e.index, e.term, cmd_str);
                            w.write_all(s.as_bytes()).await?;
                        }
                    }
                }
                ...
                Some("SHOW_IMAGES") => {
                    let images = self.image_storage.read().await;
                    if images.is_empty() {
                        w.write_all(b"(empty)\n").await?;
                    } else {
                        for (id, data) in images.iter() {
                            w.write_all(format!("{} ({} bytes)\n", id, data.len()).as_bytes()).await?;
                        }
                    }
                }

                Some("UPLOAD_IMAGE") => {
                    let (photo_id, file_path) = match (parts.next(), parts.next()) {
                        (Some(id), Some(path)) => (id, path),
                        _ => {
                            w.write_all(b"Usage: UPLOAD_IMAGE <photo_id> <file_path>\n").await?;
                            continue;
                        }
                    };
                    ...
                    match self.process_image_upload(photo_id, file_path).await {
                        Ok(checksum) => {
                            // Replicate the image metadata through Raft
                            let image_data = self.image_storage.read().await;
                            if let Some(data) = image_data.get(photo_id) {
                                let image_cmd = LogCommand::StoreImage {
                                    photo_id: photo_id.to_string(),
                                    size: data.len(),
                                    checksum: checksum.clone(),
                                };
                                self.append_and_replicate(image_cmd).await;
                            }
                            
                            w.write_all(format!("UPLOAD_OK {} {}\n", photo_id, checksum).as_bytes()).await?;
                            info!("Node {}: Successfully uploaded image {}", self.id, photo_id);
                        }
                        Err(e) => {
                            let err_msg = format!("UPLOAD_ERR {}\n", e);
                            w.write_all(err_msg.as_bytes()).await?;
                            error!("Node {}: Image upload failed: {}", self.id, e);
                        }
                    }
                }
                ...
                Some("SUBMIT") => {
                    ...
                    self.append_and_replicate(LogCommand::UserOp(rest)).await;
                    w.write_all(b"OK\n").await?;
                }
    ...
    async fn append_and_replicate(&self, command: LogCommand) {
        if !matches!(*self.state.read().await, RaftState::Leader) {
            info!("Node {}: append ignored (not leader)", self.id);
            return;
        }
        ...
        info!("Node {}: appended new entry #{}", self.id, index);
