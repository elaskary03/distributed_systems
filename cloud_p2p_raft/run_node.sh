...
# Usage: ./run_node.sh <node_id> <listen_addr> <peer_list> [bootstrap_addr]
# Examples:
#   Full mesh: ./run_node.sh 1 192.168.1.100:7001 "2=192.168.1.101:7002,3=192.168.1.102:7003"
#   Bootstrap join: ./run_node.sh 4 192.168.1.104:7004 "" 192.168.1.100:7001
...
RUST_LOG=info cargo run --bin node -- \
    ...
    --addr "$LISTEN_ADDR" \
    --peers "$PEER_LIST" \
    --client-addr "127.0.0.1:$((9000 + NODE_ID))" \
    --middleware-addr "127.0.0.1:$((9100 + NODE_ID))" \
    $( [ -n "$BOOTSTRAP_ADDR" ] && echo "--bootstrap $BOOTSTRAP_ADDR" ) 2>&1 | tee "logs/node${NODE_ID}.log"